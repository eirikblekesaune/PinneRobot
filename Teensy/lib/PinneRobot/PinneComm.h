...
enum parkingProcedureState_t : uint8_t {
  ...
  PARKING_PROCEDURE_STATE_SYNC_BEFORE_STABILIZE,
...
enum command_t : uint8_t {
  ...
  CMD_TARGET_POSITION_MOVER_STATE,
  CMD_TARGET_POSITION_MOVER_PROGRESS,
  CMD_TARGET_POSITION_MOVER_MODE,
  CMD_TARGET_POSITION_MOVER_ID,
  CMD_SYNC,
  CMD_UNKNOWN
};
...
const std::map<command_t, String> CommandMap{
    ...
    {CMD_TARGET_POSITION_MOVER_STATE, "goToTargetPosition/moverState"},
    {CMD_TARGET_POSITION_MOVER_PROGRESS, "goToTargetPosition/moverProgress"},
    {CMD_TARGET_POSITION_MOVER_MODE, "goToTargetPosition/moverMode"},
    {CMD_TARGET_POSITION_MOVER_ID, "goToTargetPosition/moveId"},
    {CMD_SYNC, "sync"}};
...
const std::map<parkingProcedureState_t, String> ParkingStateMap{
  ...
  {PARKING_PROCEDURE_STATE_SYNC_BEFORE_STABILIZE, "sync_before_stabilize"},
...
public:
  ...
  NotifyTargetPositionMoverStateChange(targetPositionMoverState_t stateChange,
                                       address_t address, int32_t moveId);
  void SendParkingProceduresState(parkingProcedureState_t state, address_t address);

  void SendTargetPositionMoverProgress(float progress, address_t address, int32_t moveId);
